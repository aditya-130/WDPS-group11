...
# Disambiguate entities by finding the best candidate based on overlap and popularity
def disambiguate_entities(entities_list, weight=5):
    for ent_dict in entities_list:
        ...
        # Assign the best candidate and its score to the entity
        ent_dict["best_candidate"] = best_candidate
        ent_dict["best_score"] = best_score

    return entities_list

# Compute overlap score based on shared words between context and candidate metadata
def compute_overlap_score(context, description, alt_labels):    
    context_tokens = set(context.lower().split())  # Tokenize context
    desc_tokens = set(description.lower().split())  # Tokenize description
    alt_tokens = set()

    for lbl in alt_labels:
        alt_tokens.update(lbl.lower().split())  # Tokenize alternative labels

    # Count overlapping tokens between context and candidate metadata
...
# Add Wikidata candidates for each extracted entity
def add_candidates(entities_list):
    for ent_dict in entities_list:
        entity_text = ent_dict["text"]
        wikidata_candidates = get_wikidata_entity_candidates(entity_text)  # Query Wikidata
        ent_dict["wikidata_candidates"] = wikidata_candidates  # Add candidates
    
    return entities_list

# Extract and disambiguate entities for both question and answer
def get_entities(question, answer):
    ...
    # Add the best candidate for each question entity
    ...
    # Add the best candidate for each answer entity
